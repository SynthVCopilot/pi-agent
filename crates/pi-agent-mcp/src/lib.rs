...
/// synthv-agent-bridge 的薄封装：六个稳定 v3 工具。
pub struct SynthVBridge {
    client: McpStdioClient,
}

impl SynthVBridge {
    /// 按桥自带 .codex/config.toml 约定拉起：`node dist/src/cli.js`，工作目录为桥仓库根。
    pub async fn connect(
        bridge_repo_dir: impl Into<PathBuf>,
        env: HashMap<String, String>,
    ) -> std::result::Result<Self, McpError> {
        let spec = McpServerSpec {
            command: "node".to_string(),
            args: vec!["dist/src/cli.js".to_string()],
            working_dir: Some(bridge_repo_dir.into()),
            env,
        };
        let client = McpStdioClient::start(&spec)?;
        client.initialize("pi-agent", "0.1.0").await?;
        Ok(Self { client })
    }

    pub async fn call(&self, tool: &str, args: Value) -> std::result::Result<Value, McpError> {
        self.client.call_tool(tool, args).await
    }

    pub async fn status(&self) -> std::result::Result<Value, McpError> {
        self.call("sv_status", json!({})).await
    }
    pub async fn describe(&self, args: Value) -> std::result::Result<Value, McpError> {
        self.call("sv_describe", args).await
    }
    pub async fn query(&self, args: Value) -> std::result::Result<Value, McpError> {
        self.call("sv_query", args).await
    }
    pub async fn command(&self, args: Value) -> std::result::Result<Value, McpError> {
        self.call("sv_command", args).await
    }
    pub async fn ui(&self, args: Value) -> std::result::Result<Value, McpError> {
        self.call("sv_ui", args).await
    }
    pub async fn review(&self, args: Value) -> std::result::Result<Value, McpError> {
        self.call("sv_review", args).await
    }
}
