    ...
    fn step(&self, conversation: &[ChatMessage], tools: &[ToolDefinition]) -> Result<AgentStep> {
        ...
        match response {
            Ok(resp) => {
                let body = resp
                    .into_string()
                    .map_err(|e| PiError::new(format!("读响应失败: {e}")))?;
                Self::parse_response(&body)
            }
            Err(ureq::Error::Status(code, resp)) => {
                let body = resp.into_string().unwrap_or_default();
                let snippet: String = body.chars().take(600).collect();
                Err(PiError::new(format!("HTTP {code}: {snippet}")))
            }
            Err(e) => Err(PiError::new(format!("请求失败: {e}"))),
        }
    }
}
...
impl PiConfig {
    ...
    /// 按配置构造 provider。
    pub fn build_provider(&self) -> Result<Box<dyn AgentProvider>> {
        match self.provider.as_str() {
            "anthropic" => {
                let cfg = self
                    .anthropic
                    .clone()
                    .ok_or_else(|| PiError::new("provider=anthropic 但缺少 anthropic 配置段"))?;
                Ok(Box::new(AnthropicProvider::new(cfg)))
            }
            "echo" => Ok(Box::new(pi_agent_core::EchoProvider)),
            other => Err(PiError::new(format!("未知 provider: {other}"))),
        }
    }
}
...
mod tests {
    ...
    /// 真实网络冒烟：需要 PI_TEST_BASE_URL / PI_TEST_TOKEN / PI_TEST_MODEL 环境变量。
    /// 平时 ignore，显式 `cargo test -- --ignored` 执行。
    #[test]
    #[ignore]
    fn live_relay_roundtrip() {
        let base = std::env::var("PI_TEST_BASE_URL").expect("PI_TEST_BASE_URL");
        let token = std::env::var("PI_TEST_TOKEN").expect("PI_TEST_TOKEN");
        let model = std::env::var("PI_TEST_MODEL").unwrap_or_else(|_| "glm-5.2".into());
